...
//버튼의 크기 베리언트 : small, medium, large 세 글자중 하나만 가능
...
//디자이너가 정해둔 특정 단어(상태)만 입력 받도록 유니온 타입을 사용합니다
interface AlertProps{
  message:string; //어떤 메시지를 띄울 것인지?
...
export default function AlertBox(props:AlertProps){
 ...
 if(props.status === 'error'){
  return <div style={{color:'red', fontWeight:'bold'}}>에러 : {props.message}</div>
 }
 //위에서 로딩과 에러를 걸러냈으므로
 //여기 도달한 status는 무조건 success
 return <div style={{color:'green'}}>성공 : {props.message}</div>
}